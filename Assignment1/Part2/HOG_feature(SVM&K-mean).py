...
def extract_hog_features(X, pixels_per_cell=(4,4), cells_per_block=(2,2), orientations=9):
    """
    Extract HOG features from all images in X.
...
for n_clusters in n_clusters_list:
    ...
    preds, acc, t = kmeans_classifier(X_train_hog, y_train, X_test_hog, n_clusters)
...
for kernel in svm_kernels:
    ...
    preds, acc, t = svm_classifier(X_train_hog, y_train, X_test_hog, kernel_type=kernel)
...
plt.ylabel("Actual")
plt.title(f"K-Means (HOG) - {best_kmeans['clusters']} clusters, Acc: {best_kmeans['accuracy']:.2f}%")
...
plt.xlabel("Predicted")
plt.ylabel("Actual")
plt.title(f"SVM (HOG) - {best_svm['kernel']} kernel, Acc: {best_svm['accuracy']:.2f}%")
