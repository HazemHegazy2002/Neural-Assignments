...
RASAIF_BASE_URL = (
    "https://huggingface.co/datasets/ImruQays/"
    "Rasaif-Classical-Arabic-English-Parallel-texts/resolve/main/"
)
RASAIF_FILES = [
    "A_Muslim_Manual_of_War.csv",
    "Al-Hanin_Ila-l-Awtan.csv",
    "Avarice_and_the_Avaricious.csv",
    "Contemplation.csv",
    "Diseases_of_the_Hearts_and_Their_Cures.csv",
    "Nahj_al-Balagha.csv",
]
...
def fetch_rasaif_classical_entries(limit: int = 27) -> list:
    entries = []
    for filename in RASAIF_FILES:
        if len(entries) >= limit:
            break
        url = f"{RASAIF_BASE_URL}{filename}"
        response = requests.get(url, headers={"Range": "bytes=0-900000"}, timeout=60)
        response.raise_for_status()
        text = response.content.decode("utf-8", errors="ignore")
        reader = csv.DictReader(io.StringIO(text))
        for row in reader:
            paragraph = clean_text(row.get("ar", ""))
            if len(paragraph) < 80:
                continue
            entries.append({
                "type": "Classical",
                ...
                "paragraph": paragraph,
                "source": "Rasaif-Classical-Arabic-English",
                "source_detail": url,
            })
            if len(entries) >= limit:
                break
...
def fetch_classical_entries(limit: int = 27) -> list:
    ...
    if len(entries) < limit:
        entries.extend(fetch_rasaif_classical_entries(limit - len(entries)))
...
def build_real_source_entries() -> list:
    ...
    classical_count = sum(1 for entry in entries if entry.get("type") == "Classical")
    if classical_count < 27:
        needed = 27 - classical_count
        fallback = [item for item in PARAGRAPHS if item.get("type") == "Classical"]
        for item in fallback[:needed]:
            entries.append({
                "type": "Classical",
                "topic": item.get("topic", "Classical"),
...
def save_dataset(dataset: list, path: str = "dataset.json"):
    ...
    print(f"  [OK] Saved {len(dataset)} entries -> {path}")
...
if __name__ == "__main__":
    ...
    # Quick sanity check — avoid printing Arabic in limited terminals
    ...
    for item in dataset[:3]:
        ...
        for i, ann in enumerate(item["annotations"], 1):
            print(f"    Ann{i}: {len(ann)} keyphrases")
        print(f"    Gold (majority vote): {len(item['gold_keyphrases'])} keyphrases")